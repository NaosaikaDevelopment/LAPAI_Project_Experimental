...
SUM_MODEL_NAME = "Qwen2.5-0.5B-Instruct-CPU"#<---this is model for sumerize, be sure that model is light weight if you wanna change
...
def learning_core():
    ...
    while True:
        ...
        if not session_file:
            print("No sesion to Learning, resting...")
            time.sleep(60)
            continue
        history = load_history(session_file)
...
def proccess_Running_AI_Online_mode(): 
    ...
    try:
        ...
        print("[INFO] Got an error to start research")
