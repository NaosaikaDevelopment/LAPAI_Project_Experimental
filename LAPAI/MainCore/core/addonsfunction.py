...
def normalize_fts(bm25_score):
    # SQLite FTS5 BM25 is ordered with lower scores being more relevant.
    # FTS5 normally returns negative BM25 values, so map relevance to (0, 1].
...
def trim_prompt(prompt,memory_prompt=None,chat_history=None,max_tokens=None):
    # Emergency fallback only. Memory should normally be managed by
    # active-memory filtering, compaction, and targeted recall.
