from .state import *
def recall_from_faiss(query_vector,topk=5,threshold=cache.conf.get('thresholdrfs'),meta_type=None,session_id=None,):
    ...
    if meta_type is not None or session_id is not None:
        candidate_k = cache.faiss_index.ntotal
    else:
        candidate_k = min(
            max(topk * 5, 30),
...
def add_to_faiss(index,id_map,text,uid,meta_type="conversation",importance=1.0,session_id=None,memory_id=None,):
    ...
    # The FAISS row number is the map key.
    new_id = index.ntotal
